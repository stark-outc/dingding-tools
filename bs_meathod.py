...
def ks_plot_self(y, prob, title, bins=20, return_detail=False):
    ...
    data.sort_values('prob', ascending=True, inplace=True)
...
def get_bayes_param(train_data, bayes_params=None):
    ...
    def rfc_cv(learning_rate, n_estimators, max_depth,
        ...
        estimator = xgb.XGBClassifier(n_estimators=int(n_estimators), max_depth=int(max_depth),
                                      min_child_weight=int(min_child_weight), gamma=int(gamma),
        ...
        cval = cross_val_score(estimator, training[features], training.y, scoring='roc_auc', cv=5)
        return cval.mean()
